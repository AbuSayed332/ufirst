...
// DATA MAPPED DIRECTLY FROM uFIRST CORPORATE PROFILE
const uFirstDivisions = [
  { 
    id: 'soft-dev', 
    name: 'Software Design', 
    icon: '💻', 
    details: 'Custom Application & Desktop Software',
  ...
  { 
    ...
    icon: '🗄️', 
    details: 'High-Performance Data Structuring',
  ...
  { 
    ...
    name: 'ERP Solutions', 
    icon: '⚙️', 
    details: 'Custom Enterprise Resource Planning Modules',
  ...
  { 
    ...
    name: 'E-Commerce', 
    icon: '🛒', 
    details: 'B2B/B2C Portals & Secure Payments',
    metrics: 'Attributes Control' 
  },
  { 
    id: 'digi-mkt', 
    name: 'Digital Marketing', 
    icon: '📈', 
    details: 'SEO, SMM & Social Media Optimization',
  ...
  { 
    ...
    icon: '🌐', 
    details: 'Custom Web Requirements & Gateway Portals',
...
export const CommandHub: React.FC<CommandHubProps> = ({ onNodeClick }) => {
  ...
  useEffect(() => {
    ...
    setTelemetry(Array.from({ length: 20 }, () => 20 + Math.random() * 80));
    return () => window.removeEventListener('resize', checkMobile);
  }, []);

  return (
    <section id="hub" className="py-12 md:py-24 relative overflow-hidden bg-slate-50 dark:bg-[#020617] w-full">
      <div className="max-w-[1600px] mx-auto px-8 md:px-16 relative z-10">
        
        {/* Header Section */}
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between mb-16 md:mb-24 gap-10">
           <div className="max-w-4xl">
              <div className="flex items-center gap-6 mb-8">
                 <div className="w-20 h-2 bg-cyan rounded-full shadow-vibrant" />
                 <span className="text-cyan font-black text-xs md:text-sm uppercase tracking-[1em]">Our Services</span>
              </div>
              <h2 className="text-5xl md:text-8xl font-display font-black text-slate-900 dark:text-white leading-[0.85] tracking-tighter italic">
                uFirst Limited <br/><span className="text-vibrant"> Services.</span>
              </h2>
           </div>
           
           {/* {!isMobile && (
             <div className="p-8 glass-card rounded-[40px] min-w-[340px] border border-white/5 hidden lg:block">
                <div className="flex justify-between items-center mb-6">
                  <div className="space-y-1">
                     <div className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Neural_Sync_Stream</div>
                     <div className="text-2xl font-display font-black text-brand dark:text-cyan italic">OPTIMAL_SYNC</div>
                  </div>
                  <motion.div animate={{ opacity: [1, 0.5, 1] }} transition={{ duration: 1.5, repeat: Infinity }} className="w-4 h-4 rounded-full bg-green-500 shadow-[0_0_15px_#22c55e]" />
                </div>
                <div className="flex gap-1 h-16 items-end">
                   {telemetry.map((t, i) => (
                     <motion.div key={i} animate={{ height: `${t}%` }} className="flex-1 bg-cyan/40 rounded-t-sm" />
        ...
        {isMobile ? (
          /* MOBILE VIEW: Informative Grid */
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {uFirstDivisions.map((biz, idx) => (
              <motion.button 
                key={biz.id}
                whileTap={{ scale: 0.98 }}
                onClick={() => onNodeClick(biz.id)}
                className="flex flex-col items-start gap-4 p-8 bg-white dark:bg-slate-900 rounded-[35px] border border-slate-100 dark:border-white/5 shadow-giant text-left"
              >
                <span className="text-5xl">{biz.icon}</span>
                <div>
                   <span className="block text-lg font-black uppercase tracking-tight text-slate-900 dark:text-white italic">{biz.name}</span>
                   <p className="text-xs text-slate-400 mt-2 font-medium">{biz.details}</p>
                </div>
              </motion.button>
            ))}
          </div>
        ) : (
          /* DESKTOP VIEW: Orbital Master Hub with Logo Integration */
          <div className="relative h-[850px] flex items-center justify-center">
            
            {/* LOGO MASTER NODE - Replace SVG with your actual Logo component if available */}
            <motion.div 
              animate={{ scale: [1, 1.02, 1], rotate: [0, 2, 0, -2, 0] }}
              transition={{ duration: 10, repeat: Infinity }}
              className="w-72 h-72 rounded-full bg-brand flex items-center justify-center border-[12px] border-white/10 shadow-[0_0_150px_rgba(0,173,239,0.3)] relative z-20"
            >
              <div className="text-center">
                 <div className="text-[10px] font-black text-white/40 uppercase tracking-[0.6em] mb-3">uFirst</div>
                 {/* Integrated Logo Text */}
                 <div className="text-7xl font-display font-black text-cyan-600 italic tracking-tighter leading-none"><img src="./assets/images/logo1.png" alt="logo" /></div>
                 <div className="text-[8px] font-mono text-cyan mt-4 tracking-[0.4em] opacity-80">A PLACE OF EXCELLENCE</div>
              </div>
            </motion.div>

            {/* Strategic Information Nodes */}
            {uFirstDivisions.map((biz, idx) => {
              const angle = (idx / uFirstDivisions.length) * Math.PI * 2;
              const radius = 380;
              const x = Math.cos(angle) * radius;
              const y = Math.sin(angle) * radius;
              const isActive = activeNode === biz.id;
              ...
              return (
                <div key={biz.id} className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <motion.div 
                    ...
                    className="group cursor-pointer flex flex-col items-center pointer-events-auto"
                  >
                    <div className={`w-32 h-32 rounded-[40px] bg-white dark:bg-slate-900 border-4 transition-all duration-500 flex flex-col items-center justify-center shadow-2xl relative z-10 ${
                      isActive ? 'border-cyan scale-110 shadow-vibrant' : 'border-slate-100 dark:border-white/5'
                    }`}>
                      <span className="text-5xl mb-2">{biz.icon}</span>
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{biz.id.replace('-', '_')}</span>
                    </div>
                    ...
                    <AnimatePresence>
                      {isActive && (
                        <motion.div 
                          ...
                          animate={{ opacity: 1, y: 0 }} 
                          exit={{ opacity: 0, y: 15 }} 
                          className="absolute top-full mt-6 glass-card p-6 rounded-[30px] border border-cyan/40 min-w-[240px] text-center z-40 shadow-giant"
                        >
                          <div className="text-xl font-display font-black text-slate-900 dark:text-white uppercase italic tracking-tight">{biz.name}</div>
                          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2 font-medium leading-relaxed">{biz.details}</p>
                          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-white/5 flex items-center justify-center gap-2">
                             <div className="w-2 h-2 rounded-full bg-cyan animate-pulse" />
                             <span className="text-[9px] font-mono text-cyan uppercase tracking-widest">{biz.metrics}</span>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                    ...
                    <svg className="absolute -z-10 w-[1000px] h-[1000px] pointer-events-none overflow-visible">
                      <motion.line 
                        x1="0" y1="0" x2={-x} y2={-y} 
                        stroke={isActive ? "#00adef" : "rgba(0,173,239,0.08)"}
                        strokeWidth={isActive ? "4" : "1"}
